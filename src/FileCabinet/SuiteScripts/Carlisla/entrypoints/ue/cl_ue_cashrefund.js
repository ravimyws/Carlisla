 ...
 */
    (query, record, runtime) => {
        ...
        const afterSubmit = (scriptContext) => {
            ...
            try {
                ...
                if (scriptContext.type === scriptContext.UserEventType.CREATE || scriptContext.type === scriptContext.UserEventType.COPY) {
                    ...
                    let returnAuthorization = getRecord(record.Type.RETURN_AUTHORIZATION, createdFrom,true);
                    createdFrom = returnAuthorization.getValue(constants.RECORDS.INVOICE.FIELDS.CREATED_FROM);
                    let cashSale = getRecord(record.Type.CASH_SALE, createdFrom,true);
                    createdFrom = cashSale.getValue(constants.RECORDS.INVOICE.FIELDS.CREATED_FROM);
                        ...
                        if (isAgency) {
                            ...
                            if (vendor === vendorIOPAgencyOwner) {
                                if (partnersData.isFifty) {  
                                    ...
                                    let vbid = createVC(transactionObj.id, primaryCommissionSplit, partnersData.fiftyPercentPartner.id, brand,seasonIop);
                                    let vbid2 = createVC(transactionObj.id, primaryCommissionSplit, vendorIOPAgencyOwner, brand,seasonIop);
                                    savedVBS.push(vbid);
                                    savedVBS.push(vbid2);
                                    
                                }else{
                                    let vbid = createVC(transactionObj.id, primaryCommission, partner, brand,seasonIop);
                                    savedVBS.push(vbid);
                                }
                                ...
                                if (partnerSplit100percent) {
                                    ...
                                    let primaryCommissionSplit = primaryCommission * partnerSplit;
                                    let vbid = createVC(transactionObj.id, primaryCommissionSplit, partner, brand,seasonIop);
                                    let vbid2 = createVC(transactionObj.id, primaryCommissionSplit, vendorIOPAgencyOwner, brand,seasonIop);
                                    savedVBS.push(vbid);
                                    savedVBS.push(vbid2);
                                } else {
                            ...
                            if(!associateCommission){
                                ...
                                let vbid = createVC(transactionObj.id, primaryCommission, vendorIOPAgencyOwner, brand,seasonIop);
                                savedVBS.push(vbid);
                                
                            }
                            else if (associateCommission === primaryCommission) {
                                let vbid = createVC(transactionObj.id, associateCommission, associate, brand,seasonIop);
                                savedVBS.push(vbid);
                            } else if (associateCommission < primaryCommission) {
                                ...
                                if (partnersData.isFifty) {
                                    ...
                                    if (vendorIOPAgencyOwner == partnersData.fiftyPercentPartner.id) {
                                        let vbid2 = createVC(transactionObj.id, (primaryCommission - associateCommission), vendorIOPAgencyOwner, brand,seasonIop);
                                        savedVBS.push(vbid2);
                                    } else {


                                        let vbid2 = createVC(transactionObj.id, (primaryCommission - associateCommission) * 0.5, vendorIOPAgencyOwner, brand,seasonIop);
                                        let vbid3 = createVC(transactionObj.id, (primaryCommission - associateCommission) * 0.5, partnersData.fiftyPercentPartner.id, brand,seasonIop);
        ...
        function getItemFromParameter() {
            ...
            let item = scriptObj.getParameter({ name: 'custscript_cl_ue_cashrefund_item' });
            return item;
        }
        ...
        function createVC(tranId, primaryCommission, vendorId, brand,seasonIop) {
            log.error('createVB');
